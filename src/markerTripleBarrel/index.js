...
function postNamazuCommand(str, then, error) {
...
function markingAttack(index) {
  ...
  if (markingList.length === 3) {
    ...
    for (let i = 0; i < markingList.length; i++) {
      ...
      postNamazuCommand(`/marking attack${i + 1} <${member.partyIndex}>`);
    }
    markingList = [];
...
function handleTest() {
  log("开始测试，覆写测试数据...", LogEnum.MESSAGE);
  handlePartyChanged(fakeParty, 0);
  log("覆写小队列表...成功！", LogEnum.MESSAGE);
  playerID = fakeParty[4].id;
  log("覆写当前玩家为:" + playerID + fakeParty[4].name + "...成功！", LogEnum.MESSAGE);
  setTimeout(() => {
    const testLog = [
    ...
    testLogLine(handleLogLine, testLog, 1);
  }, 1000);
  setTimeout(() => {
